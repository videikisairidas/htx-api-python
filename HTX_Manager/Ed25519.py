...
private_key_pem_path = "htx_api_python/HTX_Manager/_createED25519/ed25519-private.pem"
password = None
...
class HTX_Api_Manager:
    # BASE
    def __init__(self, access_key, url="https://api.huobi.pro"):
        if not access_key or not private_key_pem_path:
            raise ValueError("API Access Key and Private Key Path cannot be empty.")
        
        self.access_key = access_key
        self.secret_key: Ed25519PrivateKey = cast(
            Ed25519PrivateKey,
            load_pem_private_key(data, password=password)
        )
        self.BASE_URL = url
    ...
    async def get_account_balance(self, account_id: Any) -> Optional[Dict[str, Any]]:
        """Fetches the balance for a specific account."""
        return await self._signed_request('GET', f'/v1/account/accounts/{account_id}/balance')